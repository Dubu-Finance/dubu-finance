...
pub struct MakerParams {
    ...
    /// Largest base leg a single order may commit.
    pub max_base_per_order: u128,
...
impl MakerParams {
    ...
    #[must_use]
    pub fn half_spread_e2(&self, sigma_millibps: u32) -> u32 {
        let scaled = u64::from(self.base_half_spread_e2)
            + u64::from(self.sigma_coefficient_e2) * u64::from(sigma_millibps) / 1_000;
...
pub struct MarketState {
    ...
    /// Current volatility estimate, shared with the curve.
    pub sigma_millibps: u32,
...
pub enum Refusal {
    ...
    UnknownPair,
    /// Zero, or more than `max_base_per_order`.
    SizeOutOfRange,
...
impl Book {
    ...
    /// Prices `base_amount` of base, reserves it, and returns the order to sign.
    ///
    /// `taker_buys_base` is the taker's direction: true when the taker wants base out of the
    /// maker, which is the maker selling base.
    ///
    /// # Errors
    ///
    ...
    pub fn quote(
        ...
        if base_amount == 0 || base_amount > params.max_base_per_order {
        ...
        // fair, buying base is priced down from it — the sign is not configurable, and writing it
        // out twice rather than as `fair * (1 ± s)` keeps the rounding on the maker's side in both
        // directions.
        let num = u128::from(BPS_E2) * 100;
        let quote_amount = if taker_buys_base {
            let price = state.fair.checked_mul(num + u128::from(half)).ok_or(Refusal::Undefined)? / num;
            // Round the taker's payment up.
            base_amount
                .checked_mul(price)
            ...
            let price = state.fair.checked_mul(num - u128::from(half)).ok_or(Refusal::Undefined)? / num;
            // Round the maker's payment down.
            base_amount.checked_mul(price).ok_or(Refusal::Undefined)? / scale
        };
        if quote_amount == 0 {
            return Err(Refusal::SizeOutOfRange);
        }
        ...
        Ok(Quote {
            ...
            maker_amount: if taker_buys_base { base_amount } else { quote_amount },
            taker_amount: if taker_buys_base { quote_amount } else { base_amount },
...
mod tests {
    ...
    fn state() -> MarketState {
        MarketState {
            ...
            // $2,000 with an 18/6 pair: priceScaleExp 24, so 2000 * 10^24 / 10^18 ... the pool's
            // own units. Concretely the value `Deploy` prints for this market.
    ...
    fn the_spread_is_charged_against_the_maker_in_both_directions() {
        ...
        let buy = book.quote(&params(), &state(), true, ONE_ETH, 1_000).expect("quotable");
        let sell = book.quote(&params(), &state(), false, ONE_ETH, 1_000).expect("quotable");

        // Taker buys 1 base: pays more than mid. Taker sells 1 base: receives less than mid.
        let mid = state().fair * ONE_ETH / 10u128.pow(24);
        assert!(buy.taker_amount > mid, "buying must cost above mid");
        assert!(sell.maker_amount < mid, "selling must pay below mid");
    }
    ...
    fn a_round_trip_through_both_sides_always_loses() {
        ...
        let buy = book.quote(&params(), &state(), true, ONE_ETH, 1_000).expect("quotable");
        let sell = book.quote(&params(), &state(), false, ONE_ETH, 1_000).expect("quotable");
        // Buy a base for `buy.taker_amount`, sell it back for `sell.maker_amount`.
        assert!(sell.maker_amount < buy.taker_amount, "the round trip must not be free money");
    }

    #[test]
    fn volatility_widens_the_spread_up_to_the_cap() {
        let calm = params().half_spread_e2(0);
        let choppy = params().half_spread_e2(50_000);
        assert!(choppy > calm);
        assert_eq!(params().half_spread_e2(u32::MAX), params().max_half_spread_e2);
    }

    #[test]
    fn a_size_above_the_per_order_cap_is_refused() {
        let mut book = Book::new();
        let too_big = params().max_base_per_order + 1;
        assert_eq!(book.quote(&params(), &state(), true, too_big, 1_000), Err(Refusal::SizeOutOfRange));
    ...
    fn a_reservation_stops_the_same_inventory_being_promised_twice() {
        ...
        book.quote(&params(), &s, true, 6 * ONE_ETH, 1_000).expect("first fits");
        assert_eq!(book.reserved(1, true), 6 * ONE_ETH);
        // Only 4 left, so a second 6 must be refused rather than double-promised.
        assert_eq!(
            book.quote(&params(), &s, true, 6 * ONE_ETH, 1_000),
            Err(Refusal::InsufficientInventory)
        );
        assert!(book.quote(&params(), &s, true, 4 * ONE_ETH, 1_000).is_ok());
    }
    ...
    fn a_reservation_is_released_when_the_order_can_no_longer_be_filled() {
        ...
        let mut s = state();
        s.base_balance = 10 * ONE_ETH;
        book.quote(&params(), &s, true, 10 * ONE_ETH, 1_000).expect("fits");
        assert_eq!(book.open_len(), 1);

        // Still live one second before expiry.
        book.expire(1_000 + params().ttl_secs - 1);
        assert_eq!(book.reserved(1, true), 10 * ONE_ETH);

        book.expire(1_000 + params().ttl_secs + 1);
        assert_eq!(book.reserved(1, true), 0);
        assert!(book.quote(&params(), &s, true, 10 * ONE_ETH, 2_000).is_ok());
    }
    ...
    fn the_curves_epoch_is_subtracted_before_rfq_may_quote() {
        ...
        assert_eq!(
            book.quote(&params(), &s, true, 5 * ONE_ETH, 1_000),
            Err(Refusal::InsufficientInventory)
        );
        assert!(book.quote(&params(), &s, true, ONE_ETH, 1_000).is_ok());
    }
    ...
    fn nonces_do_not_repeat_within_a_run() {
        ...
        book.quote(&params(), &state(), true, ONE_ETH, 1_000).expect("quotable");
