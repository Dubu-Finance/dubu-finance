...
//
// Everything between here and the `MockPyth` banner is a faithful subset of `pyth-sdk-solidity`
// — same type names, same field names, same field order, same error names, same function
// selectors. It is declared here rather than imported because the SDK is not vendored into this
// repo and this file is the only consumer so far. When `PropPool` lands and needs the reference
// bound, lift these three declarations verbatim into `src/interfaces/IPyth.sol` and have both
// the pool and this mock import them; nothing about them is mock-specific.
//
...
//
//     IPyth pyth = IPyth(address(new MockPyth(60, 0)));
//   to
